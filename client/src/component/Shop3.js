...
const Home = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:8001/getusersShop3")
      .then((res) => {
  ...
  //   const getdata = async () => {
  //     try {
  //       const res = await fetch("/getusersShop3", {
  //         method: "GET",
  //         headers: {
  ...
  const deleteuser = async (id) => {
    try {
      const res2 = await fetch(`/deleteuserShop3/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <>
      ...
      <div className="mt-5">
        <div className="container">
          ...
          <NavLink to="/Shop3Register" className="btn btn-primary mb-3 ">
              Add data
          </NavLink>
              ...
              return (
                <tr key={index}>
                  ...
                  <td className="d-flex justify-content-between">
                    <NavLink to={`Shop3view/${getUserdata.id}`}>
                      <button className="btn btn-success">
                        <VisibilityIcon />
                      </button>
                    </NavLink>
                    <NavLink to={`Shop3edit/${getUserdata.id}`}>
                      {" "}
